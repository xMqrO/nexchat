...
storage.ensure();
const demoUsers = [
  { id: 'NC-482913', username: 'Ahmed', password: 'password', avatar: '', bio: 'Product designer & coffee enthusiast.', color: '#8b7cf6', createdAt: new Date().toISOString() },
  { id: 'NC-735192', username: 'Mohamed', password: 'password', avatar: '', bio: 'Building quietly, learning loudly.', color: '#32c5d2', createdAt: new Date().toISOString() },
  { id: 'NC-918624', username: 'Sara', password: 'password', avatar: '', bio: 'Creative soul. Say hello!', color: '#ff8a65', createdAt: new Date().toISOString() },
  { id: 'NC-204817', username: 'Youssef', password: 'password', avatar: '', bio: 'Always up for a good conversation.', color: '#65d38a', createdAt: new Date().toISOString() }
];
...
async function hydrateConversation(c, userId, online, typing) {
  ...
  return { ...c, other: safeUser(findUser(otherId) || { id: otherId, username: 'Unknown', color: '#8b7cf6' }, online), lastMessage: convMessages[0] || null, unread: convMessages.filter((m) => m.senderId !== userId && m.status !== 'read').length, typing: typ };
}
...
app.put('/api/profile', requireUser, async (req, res) => {
  ...
  u.username = username; u.bio = clean(req.body.bio ?? u.bio, 160);
